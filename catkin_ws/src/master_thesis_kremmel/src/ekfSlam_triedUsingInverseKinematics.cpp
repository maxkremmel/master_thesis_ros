//ToDo: Funktionen verwenden wo es Sinn macht zB beim berechnen von yaw aus Quaternionen
//ToDo: Variablennamen versinnvollern
...
public:
    EKFSlam(ros::NodeHandle &nTemp) : n(nTemp)
    {
        ...
        last_timestamp = ros::Time::now().toSec(); // Übernehemend der aktuellen Zeit
     ...
     void secondMotionModel(const sensor_msgs::ImuConstPtr &new_imu_msg)
    {
        double dt = new_imu_msg->header.stamp.toSec() - last_timestamp_2;
        last_timestamp_2 = new_imu_msg->header.stamp.toSec();

        vel(0) += new_imu_msg->linear_acceleration.x * dt;
        vel(2) = new_imu_msg->angular_velocity.z;
        double v = 0;
        if (abs(vel(0)) > 0.001)
        {
            v = vel(0);
        ...
        double w = vel(2);
        double theta = state_vector_prediction(2);

        std::cout << "seq: " << new_imu_msg->header.seq << "    acc_x: " << new_imu_msg->linear_acceleration.x << "   vel_x: " << vel(0) << std::endl;

        if (abs(w) < 1e-4)
        {
            state_vector_prediction(0) += cos(theta) * v * dt;
            state_vector_prediction(1) += sin(theta) * v * dt;
            state_vector_prediction(2) += w * dt;
        }
        else
        {
            state_vector_prediction(0) += -v / w * sin(theta) + v / w * sin(theta + w * dt);
            state_vector_prediction(1) += +v / w * cos(theta) - v / w * cos(theta + w * dt);
            state_vector_prediction(2) += w * dt;
        }

        nav_msgs::Odometry predictionState;
        tf2::Quaternion quat;
        quat.setRPY(0, 0, state_vector_prediction(2));
        geometry_msgs::Pose predictedPose;
        predictedPose.position.x = state_vector_prediction(0);
        predictedPose.position.y = state_vector_prediction(1);
        predictedPose.position.z = 0;
        predictedPose.orientation = tf2::toMsg(quat);

        predictionState.header.frame_id = "odom";
        predictionState.header.stamp = ros::Time::now();
        predictionState.child_frame_id = "base_link";
        predictionState.pose.pose = predictedPose;

        predictionStatePublisher.publish(predictionState);
    }
    ...
    void predict(const nav_msgs::OdometryConstPtr &new_pose_msg, const sensor_msgs::PointCloud2ConstPtr &new_laserscan_msg)
    {
        ...
        last_timestamp = ros::Time::now().toSec();
        tf2::Quaternion q(
            new_pose_msg->pose.pose.orientation.x,
            new_pose_msg->pose.pose.orientation.y,
            new_pose_msg->pose.pose.orientation.z,
            new_pose_msg->pose.pose.orientation.w);
        tf2::Matrix3x3 m(q);
        m.getRPY(roll, pitch, yaw);

        lidar_odom << new_pose_msg->pose.pose.position.x, new_pose_msg->pose.pose.position.y, yaw; // aktuelle Pose -> berechnet von KISS-ICP
        
        Eigen::Vector3d vel3d(3);
        vel3d = (lidar_odom_last - lidar_odom) / dt;
        // Hier weiter machen... v richtig berechnen
        ...
        lidar_odom_last = lidar_odom;

        Eigen::Vector3d mu_last = state_vector.head(3);
...
private:
    ...
    Eigen::Vector3d lidar_odom_last;
    int DIMsize;
    double alpha1;
    double alpha2;
    double alpha3;
    ...
    double roll, pitch, yaw;           // Roll, Pitch und Yaw der Odometry Message von KISS
